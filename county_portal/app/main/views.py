...
if current_user.county_id:                                                
            permit_types = PermitType.query.join(Department)\
                .filter(Department.county_id == current_user.county_id, PermitType.
active == True)\
                .limit(6).all()                                                   

stats = {                                                                 
            'total_applications': len(applications),                              
            'pending_applications': len([app for app in applications if app.status
  in ['Submitted', 'Under Review']]),                                             
            'approved_applications': len([app for app in applications if app.     
  status == 'Approved']),                                                         
            'rejected_applications': len([app for app in applications if app.     
  status == 'Rejected'])                                                          
        }
